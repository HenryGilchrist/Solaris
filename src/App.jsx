...
function App() {
  ...
  const [bgClass,setBgClass] = useState({ middle: false, new: false});
  const [bgColour,setBgColour] = useState({current: "var(--colour1-end)", middle: "", new: ""});
  const [buttonColour,setButtonColour] = useState("var(--button-colour1");
  const button = useRef(null);
  const logo = useRef(null);
  const nav = useRef(null);
  const contentBox = useRef(null);
  const root = document.documentElement;
  const canChangeDelay = 2000;
  const [canInBounds,setCanInBounds] = useState(true);
  const disabledColor = "#AAA";
  const product = useRef(null);
  const [drinkInfo,setDrinkInfo] = useState([]);
  const drinkInfoHeader = ['drink-name','description','added'];
  const drinkInfoBody = ["Calories",'Sugars',"Sodium","Potassium","Magnesium","special"];
  const [infoIndex,setInfoIndex] = useState(0);

  useGSAP(() => {
      const sloganLines = document.querySelectorAll(".slogan-line");
      sloganLines.forEach((sloganLine, i) => {
        gsap.fromTo(sloganLine,
          {
            y: 0
          },
          {
  ...
  useEffect(() => {
    ...
    setDrinkInfo(data['drink-info']);
    console.log(data['drink-info']);
    ...
    const handleScroll = () => {
      ...
      if (yScroll > 10) {
        navBorders.forEach(border => border.classList.add("closed"));
        logo.current.classList.add("small");
        nav.current.classList.add("closed");

        if (yScroll + window.innerHeight >= bodyHeight - 2){
          setCanInBounds(c => !c ? true : c);

        } 
        else {
          setCanInBounds(c => c ? false : c);
        }
      } else if (yScroll <= 10) {

        setCanInBounds(c => !c ? true : c);
        navBorders.forEach(border => border.classList.remove("closed"));
        logo.current.classList.remove("small");
  ...
  async function changes(){
    ...
    const currentCan = canIndex;
    const nextCan = currentCan == 2 ? 0 : currentCan + 1;

    

    setCanClass(arr => arr.map((n,i) => i == currentCan ? "leaving" : ""));

    setRise(r => !r);

    await sleep(canChangeDelay);

    setCanIndex(nextCan);
    setCanClass(arr => arr.map((n,i) =>  i == nextCan ? "current" : ""));

    const newColour = `var(--colour${nextCan+1}-end)`;
    const middleColour = `var(--colour${nextCan+1}-middle)`;
    setBgColour(colourList => ({...colourList, middle: middleColour, new: newColour}));
    setBgClass(classList => ({...classList, middle: true}));

    await sleep(300);
    setButtonColour(`var(--button-colour${nextCan+1})`);

    setBgClass(classList => ({...classList, new: true}));

    console.log(bgColour);
    console.log(bgClass);
    
    setRise(r => !r);
    setCanClass(arr => arr.map((n,i) =>  i == currentCan ? "" : n));

    await sleep(200);

    setInfoIndex(nextCan); // Change Info as new background sweeps over
    
    await sleep(1000);

    setBgColour(colourList => ({...colourList, current: newColour}));
    setBgClass({middle: false, new: false});
    ...
    console.log("info index change");
    async function dashBorderChange(){
      root.style.setProperty("--dash-border-opacity", "0");
      await sleep(700);
      root.style.setProperty("--dash-border-opacity", "1");
    }
  ...
  return (
    <>
      <div className="background-container">
        <div style={{backgroundColor: bgColour.current}} className={'background current'}>
        </div>
        <div style={{backgroundColor: bgColour.middle}} className={`background ${bgClass.middle ? 'grow' : ''}`}></div>
        <div style={{backgroundColor: bgColour.new}} className={`background ${bgClass.new ? 'grow' : ''}`}></div>
        ...
        <svg className="sun" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          ...
          <path opacity="0.5" d="M19.7778 4.22266L17.5558 6.25424" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/>
          <path opacity="0.5" d="M4.22217 4.22266L6.44418 6.25424" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/>
          <path opacity="0.5" d="M6.44434 17.5557L4.22211 19.7779" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/>
          <path opacity="0.5" d="M19.7778 19.7773L17.5558 17.5551" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/>
        </svg>        
      </div>

    <div className="scrolling-content" ref={contentBox}>
      <div className="content">
        <div className="slogan-container">
            <div className="slogan-line">Natural Ingredients</div>
            <div className="slogan-line">No Added Sugars</div>
            <div className="slogan-line">No Corners Cut</div>
        </div>

        <div className="button-wrapper">
          < RefreshCw className="rotate-icon" size={140} />
          <div ref={button} style={{backgroundColor: canInBounds ? buttonColour : disabledColor}} className="button" onClick={() => {if(!changeActive && canInBounds){ changes()}}}> </div>  
        </div>

          <div className="product-wrapper" ref={product}>
            <div className={`product ${rise ? 'rise' : ''}`}>
              <div style={{opacity: canIndex == 0 ? 1 : 0, "--url": "url(/Solaris/PineappleCover-crop.jpg)"}} className={`can ${canClass[0]}`}></div>
              <div style={{opacity: canIndex == 1 ? 1 : 0, "--url": "url(/Solaris/LemonCover-crop.jpg)"}} className={`can ${canClass[1]}`}></div>
              <div style={{opacity: canIndex == 2 ? 1 : 0, "--url": "url(/Solaris/OrangeCover-crop.jpg)"}} className={`can ${canClass[2]}`}></div>
            </div>
          </div>
          ...
          <div className="info-section">
              <div className="info-col left">
                <div className="drink-name-wrapper">
                    {drinkInfo.map((drink,i) => (
                      <div className={`drink-name info ${infoIndex == i ? 'visible' : ''}`}>{drink.header['drink-name']}</div>
                    ))}
                </div>
          
                <div className="description-wrapper">
                    {drinkInfo.map((drink,i) => (
                      <div className={`description info ${infoIndex == i ? 'visible' : ''}`}>{drink.header.description}</div>
                    ))}
                </div>
              </div>
              <div className="info-col right">
                <div className="drink-info-table">
                  <div className="border-wrapper-horizontal">
                    <svg className="dash-border-horizontal" preserveAspectRatio="none" viewBox="0 0 100 1" xmlns="http://www.w3.org/2000/svg">
                    <line x1="0" y1="0" x2="99" y2="0" />
                    </svg>
                    <svg className="dash-border-horizontal" preserveAspectRatio="none" viewBox="0 0 100 1" xmlns="http://www.w3.org/2000/svg">
                      <line x1="0" y1="0" x2="99" y2="0" />
                    </svg>
                  ...
                  <div className="border-wrapper-vertical">
                    <svg className="dash-border-vertical" preserveAspectRatio="none" viewBox="0 0 1 100" xmlns="http://www.w3.org/2000/svg">
                    <line x1="0" y1="0" x2="0" y2="99" />
                    </svg>
                    <svg className="dash-border-vertical" preserveAspectRatio="none" viewBox="0 0 1 100" xmlns="http://www.w3.org/2000/svg">
                      <line x1="0" y1="0" x2="0" y2="99" />
                    </svg>
                  ...
                  {drinkInfoBody.map((word, k) => (
                    <div className="drink-cell" key={k}>
                      {drinkInfo.map((drink, i) => {
                        if (!Array.isArray(drink.body[word])) {
                          return (
                            <div className={`info-cell info ${infoIndex == i ? 'visible' : ''}`} key={i}>
                              <div className="info-title">{word}</div>
                              <div className="info-value" style={{color: buttonColour}}>{drink.body[word]}</div>
                            </div>
                          );
                        }
                        else{
                          return (
                            <div className={`info-cell info ${infoIndex == i ? 'visible' : ''}`} key={i}>
                              <div className="info-title">{drink.body[word][0]}</div>
                              <div className="info-value" style={{color: buttonColour}}>{drink.body[word][1]}</div>
                            </div>
                          )
                        }
                      })}
          ...
          <svg className="frame" viewBox="0 0 573.838 573.838" preserveAspectRatio='none'>
            <g>
              ...
              <path d="c-0.226-0.33-0.336-0.717-0.495-0.521C59.991,0.062,59.99,0.031,59.989,0c-0.277,0.183-0.534,0.395-0.806,0.587     c0.183,0.042,0.366,0.085,0.548,0.129C59.827,0.739,59.924,0.759,60.02,0.783z"/>
              ...
              <ellipse cx="286.919" cy="435.378" rx="170" ry="40" />
            </g>
          </svg>
    ...
    <div className="logo" ref={logo} >Solaris</div>
    ...
    <svg className="nav-border left" viewBox='0 0 100 1' preserveAspectRatio='none' xmlns="http://www.w3.org/2000/svg">
      <line x1="0" y1="0" x2="101" y2="0"/>
    </svg>
    <svg className="nav-border right" viewBox='0 0 100 1' preserveAspectRatio='none' xmlns="http://www.w3.org/2000/svg">
